...
num.respondents.plot = ggplot(GetNonNaData('country')) +
  geom_bar(aes(x = reorder(country, country, function(x) - length(x))), fill = 'lightsalmon') +
...
summ = summary(stan.fit, pars = params)$summary
write.table(summ, file = '../mcmc_summaries/purchasing_power_summary.txt')
@
...
ggsave('../plots/purchasing_power_posterior_parameters.png', posterior.params.plot)
@
