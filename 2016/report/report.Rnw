...
se_mcmc = summ[, c('se_mean')]
n_eff = as.integer(summ[, c('n_eff')])
sampling.info.se = paste('se_mcmc:', round(se_mcmc, digits=5))
sampling.info.eff = paste('n_eff:', n_eff)
sampling.info = paste(sampling.info.se, sampling.info.eff, sep = '\n')
