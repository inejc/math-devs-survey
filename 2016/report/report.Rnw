...
y_rep = data.frame(category = pred.datasets[, 2], replication = pred.datasets[, 1], dataset = 'replicated')
y_obs = data.frame(category = rep(y, num.plots), replication = pred.datasets[, 1], dataset = 'observed')

predictive.check = rbind(y_rep, y_obs)

predictive.check.plot = ggplot(predictive.check) +
  geom_bar(aes(x = category, fill = dataset), position = 'dodge') +
